...
node.teamcity.agents.each do |name, agent| # multiple agents
  ...
  if platform?("windows")
    ...
    if platform?("mac_os_x")
      #Without this launchd will create the directory with root permissions.
      directory "#{agent.system_dir}/logs" do
        user agent.user
        group agent.group
        mode '0755'
        action :create
      ...
      file "#{agent.system_dir}/bin/mac.launchd.sh" do
        mode '0755'
      end
      
      execute 'mac launchd setup' do
        user agent.user
        command "#{agent.system_dir}/bin/mac.launchd.sh load"
        action :run
      end
    else
